...
import com.example.flower.mvvm.model.PublishPostModel;
 ...
 */
public class PublishPostViewModel extends BaseViewModel<PublishPostModel> {
    /**
     * 选择照片
     */
    ...
    public MutableLiveData<Integer> mShowEnlargePictureLiveData = new MutableLiveData<>();

    public PublishPostViewModel(@NonNull Application application) {
        super(application, new PublishPostModel());
        ...
        mAddPictureAdapter.setOnItemChildClickListener((adapter, view, position) -> {
            switch (view.getId()) {
                case R.id.ibAddPicture:
                    ...
                    break;
                case R.id.ivPicture:
                    mShowEnlargePictureLiveData.setValue(position);
                    break;
                case R.id.ibDelete:
    ...
    public List<AddPictureBean> getHaveChosenPictureList() {
        return mHaveChosenPictureList;
    }
